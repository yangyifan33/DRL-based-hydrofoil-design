...
class AirfoilEnv(gym.Env):
    ...
    def step(self, action):
        ...
        lift_coefficient, pressure_distribution = calculate_aerodynamic_properties(front_state)
...
class AirfoilEnvSD(gym.Env):
    ...
    def reset(self, random_state=None):
        ...
        if random_state:  
            ...
            Cl, pressure_distribution = calculate_aerodynamic_properties([AoA,camber])
            I = interPolate.calinterPlate(pressure_distribution)
            minum = I.compute_min()
            self.state = np.array([AoA, camber, self.target_lift_coefficient, Cl, I.compute_argmin(), minum[0]/minum[1], I.compute_middle_parameter(), 0], dtype=np.float32)  # 初始拱度和拱角
        else:
            Cl, pressure_distribution = calculate_aerodynamic_properties([5,0.1])
            I = interPolate.calinterPlate(pressure_distribution)
            minum = I.compute_min()
            self.state = np.array([5, 0.1, self.target_lift_coefficient, Cl, I.compute_argmin(), minum[0]/minum[1], I.compute_middle_parameter(), 0], dtype=np.float32)  # 初始拱度和拱角
        return self.state
    ...
    # compute the parameters about the pressure distribution
        ...
        # update the state
        self.state = np.array([front_state[0], front_state[1], self.target_lift_coefficient, lift_coefficient, I.compute_argmin(), pressure_min[0]/pressure_min[1], middle_parameter, value_is_injected], dtype=np.float32)
    ...
    def step_for_infer(self, action):
        ...
        assert self.state.shape == (8,), f"Expected self.state shape (6.3), but got {self.state.shape}"
...
class AirfoilEnv2D(gym.Env):
    ...
    # compute the parameters about the pressure distribution
        ...
        # update the state
        self.state = np.array([front_state[0], front_state[1]], dtype=np.float32)
    ...
    def step_for_infer(self, action): #step for inference,remove the computation and updating for reward 
        ...
        assert self.state.shape == (2,), f"Expected self.state shape (6.3), but got {self.state.shape}"
